...
async function executeCode(id: string, code: string, scopeId?: string): Promise<void> {
    ...
    try {
        ...
        const { runtime, skills, telegram, discord, onebot, feishu } = installCapabilityRegistry({
            ctx,
        ...
        const fh = feishu ? tracker.wrap(feishu as Record<string, unknown>) : undefined;
        ...
        const fixedArgNames = ["ctx", "runtime", "scene", "skills", "fs", "mcp", "cron", "todo", "vision", "memory", "privacy", "emergency", "dispatch", "shell", "telegram", "discord", "onebot", "qq", "feishu"];
